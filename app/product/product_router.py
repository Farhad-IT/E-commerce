...
@router.get("/{product_id}", status_code=status.HTTP_200_OK, dependencies=[Depends(get_current_user)])
...
@router.post("/{product_id}/stock", status_code=status.HTTP_200_OK, dependencies=[Depends(require_role("admin"))])
async def add_stock_to_product(
        product_id: int,
        payload: QuantitySchema,
        product_services: ProductServiceDep
) -> ProductSchema:
    return await product_services.add_stock_to_product(product_id=product_id, quantity=payload)


@router.patch("/{product_id}", status_code=status.HTTP_200_OK, dependencies=[Depends(require_role("admin"))])
async def update_product(
        product_id: int,
        payload: ProductUpdateSchema,
        product_services: ProductServiceDep
) -> ProductSchema:
    return await product_services.update_product(product_id=product_id, update_product=payload)
