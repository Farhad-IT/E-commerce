...
@router.get(
    ...
    dependencies=[
        ...
        Depends(rate_limiter(max_requests=5, window_seconds=10)),
    ],
)
...
async def get_category_by_id(
    category_id: int, cat_service: CategoryServiceDep
) -> CategorySchema:
...
async def create_category(
    payload: CategoryCreateSchema, cat_service: CategoryServiceDep
) -> CategorySchema:
...
async def update_category(
    category_id: int, payload: CategoryUpdateSchema, cat_service: CategoryServiceDep
) -> CategorySchema:
    return await cat_service.update_category(
        category_id=category_id, update_category=payload
    )
...
async def delete_category(category_id: int, cat_service: CategoryServiceDep) -> None:
