...
from app.balance.balance_schemas import BalanceSchema, BalanceCreateSchema, BalanceUpdateSchema
...
class BalanceService:
    ...
    async def create_balance(self, user_id: int, amount: BalanceCreateSchema) -> BalanceSchema:
        ...
        if not new_balance:
            new_balance = await self.balance_repository.create_balance(user_id=user_id, amount=Decimal(amount.amount))
            await self.db.commit()
            await self.db.refresh(new_balance)
        return BalanceSchema.model_validate(new_balance)

    async def update_balance(self, user_id: int, amount: BalanceUpdateSchema) -> BalanceSchema:
        new_balance = await self.balance_repository.get_by_user_id(user_id=user_id)
